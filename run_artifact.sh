...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      echo "usage: bash run_artifact.sh [--end-to-end [fp16|fp32|both]] [--microbenchmark] [--ablation] [--sensitivity] [--plots] [--all] [--frames N] [--micro-frames N] [--memory-frames N] [--warmup N] [--timing-repeats N] [--device DEVICE] [--overwrite]"
      exit 2
      ;;
  esac
  shift
...
if [[ "$run_end_to_end" == 1 ]]; then
  if [[ "$end_to_end_dtype" == "fp16" || "$end_to_end_dtype" == "both" ]]; then
    for backend in gtsparse spconv torchsparse minkowski; do
      bash run_e2e_v2.sh "$backend" fp16 "$frames" "$warmup" "$timing_repeats" "$timing_warmup" "$device" logs/end_to_end "$overwrite"
    done
  fi
  if [[ "$end_to_end_dtype" == "fp32" || "$end_to_end_dtype" == "both" ]]; then
    for backend in gtsparse spconv torchsparse minkowski; do
      [[ "$end_to_end_dtype" == "both" && "$backend" == "minkowski" ]] && continue
      bash run_e2e_v2.sh "$backend" fp32 "$frames" "$warmup" "$timing_repeats" "$timing_warmup" "$device" logs/end_to_end "$overwrite"
    done
  fi
fi
...
if [[ "$run_ablation" == 1 ]]; then
  for min_template in 0 1 4 7; do
    marker="logs/ablation/min_template_${min_template}/logs_${gpu_key}_float16_voxelnext_nuscenes_sweeps10/gtsparse.summary.json"
    run_experiment "$marker" "$frames" env GTSPARSE_MIN_TEMPLATE="$min_template" python -m gtsparse.e2e_v2.nuscenes_voxelnext \
      ...
      --log-dir "logs/ablation/min_template_${min_template}"
  done
fi
...
if [[ "$run_sensitivity" == 1 ]]; then
  for sweeps in 1 5 10 20; do
    for backend in gtsparse spconv torchsparse minkowski; do
      ...
      dtype_label=float16
      [[ "$run_dtype" == "fp32" ]] && dtype_label=float32
      marker="logs/sensitivity/logs_${gpu_key}_${dtype_label}_voxelnext_nuscenes_sweeps${sweeps}/${backend}.summary.json"
      run_experiment "$marker" "$frames" python -m gtsparse.e2e_v2.nuscenes_voxelnext --backend "$backend" --dtype "$run_dtype" \
        ...
        --warmup "$warmup" --timing-repeats "$timing_repeats" --timing-warmup-repeats "$timing_warmup" \
        --device "$device" --log-dir logs/sensitivity
    done
  done
fi

python -m experiments.aggregate --logs logs --results results
python -m experiments.plot_results --results results --figures figures
