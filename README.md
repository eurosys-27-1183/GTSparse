...
- `--spconv-tile observed|autotuned` selects the SpConv M-tile model used for the Fig 5 proportionality. `observed` (default) uses the M-tile SpConv selected in its autotune on the RTX 4090; `autotuned` reads the M-tile SpConv autotuned on the machine that produced the logs.
